...
class RabbitMQ extends EventEmitter{
    ...
    async consumer(queue, fn, options, prefetch) {
        await this.prefetch(prefetch);
        await this.startConsume(queue, fn, options);
    }

    async registerConsumer(queue, fn, options, prefetch) {
        let consumerInstance = this.consumer.bind(this, queue, fn, options, prefetch);
